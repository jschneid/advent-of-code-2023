...
def product_of_low_and_high_pulses_after_1000_button_presses(modules, pulse_queue):
    for i in range (1000):
        initial_pulse = Pulse('button', 'broadcaster', 0)
        pulse_queue.enqueue_pulse(initial_pulse)
        process_pulses_until_quiescence(modules, pulse_queue)

    return pulse_queue.product_of_low_and_high_pulses_sent()
...
print(product_of_low_and_high_pulses_after_1000_button_presses(modules, pulse_queue))
